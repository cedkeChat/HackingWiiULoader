...
static s32 startTCPGeckoThread(s32 argc, void *argv) {
	...
	if (isCodeHandlerInstalled) {
		...
		log_print("Code handler not installed (error installator)...\n");
	}
...
void startTCPGecko() {
	// log_print("Starting mods...\n");
	uint64_t titleID = OSGetTitleID();
	if(titleID == 0x0 /*Your title launch*/){

// Start mods
//code send cheat "writeKernelMemory((void *)0x0,0x0L);""
// End mods
}

	unsigned int stack = (unsigned int) memalign(0x40, 0x100);
	ASSERT_ALLOCATED(stack, "loading stacking..")
	stack += 0x100;
	void *thread = memalign(0x40, 0x1000);
	ASSERT_ALLOCATED(thread, "loading threading..")
